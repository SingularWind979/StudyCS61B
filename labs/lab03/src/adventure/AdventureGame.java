...
public class AdventureGame {
    ...
    public AdventureGame(In in) {
        this(in, new FillerStage("""
                        It's a wonderful day of learning about computer science!
                        We are going to see so many cool things today!
    ...
    public String toString() {
        StringBuilder result = new StringBuilder(this.currentStage.nextStagePrompt() + "\n");
        Map<String, AdventureStage> responses = this.currentStage.getResponses();
        if (!responses.isEmpty()) {
            result.append(">>");
            for (String response : responses.keySet()) {
                result.append(" [").append(response).append("]");
            }
        }
